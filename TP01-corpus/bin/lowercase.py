...
class LowercaserHandler(filetype.ChainedInputHandler):
    def __init__(self):
        global sent_handler
        # Bind sentence handler to `self`
        self.handle_sentence = sent_handler.__get__(self, LowercaserHandler)
    ...
    def handle_sentence_aggressive(self, sentence, ctxinfo):
...
def get_percents( ctxinfo, token_stats ) :
    """
    ...
    percents = {}
    total_count = 0
    for a_form in list(token_stats.keys()) :
    ...
    for a_form in list(percents.keys()) :
        if total_count != 0 :
            percents[ a_form ] = percents[ a_form ] / float(total_count)
        else :
            ctxinfo.warn("Percents cannot be calculated for non-occurring words!")
    return percents
