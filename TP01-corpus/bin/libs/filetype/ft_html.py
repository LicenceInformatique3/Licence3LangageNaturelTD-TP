...
class HTMLInfo(common.FiletypeInfo):
    ...
    # TODO use python-based HTML escape... Reuse our XML implementation?
...
<hr/>"""
        ...
        self.add_string(ctxinfo, html_header.format(timestamp=timestamp,
                corpusname=s[s.rfind("/")+1:],
                filename=os.path.basename(s)))


    def after_file(self, fileobj, ctxinfo):
        self.add_string(ctxinfo, "</body>\n</html>\n")
        super(HTMLPrinter, self).after_file(fileobj, ctxinfo)
    ...
    def handle_sentence(self, sentence, ctxinfo):
        self.add_string(ctxinfo, sentence.to_html(), "\n")
