...
class PlainCandidatesInfo(common.FiletypeInfo):
    ...
    comment_prefix = "#"
    escaper = PlainCorpusInfo.escaper
...
class PlainCandidatesParser(common.AbstractTxtParser):
    ...
    def _parse_line(self, line, ctxinfo):
        words = [Word(ctxinfo, {"surface":self.unescape(surface)})
                for surface in line.split("_")]
        c = self.candidate_factory.make_uniq(words)
        self.handler.handle_candidate(c, ctxinfo)
...
class PlainCandidatesPrinter(common.AbstractPrinter):
    ...
    def handle_candidate(self, candidate, ctxinfo):
        self.add_string(ctxinfo, "_".join(self.escape(w.lemma_or_surface()) \
