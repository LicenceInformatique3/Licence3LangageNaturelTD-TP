...
UNKNOWN_FEAT_VALUE = "?"  # Used by Weka
...
class FeatureSet(object):
    ...
    def __repr__(self):
        merge_op = getattr(self._merge_op, "__qualname__", self._merge_op)
        return "FeatureSet({!r}, {}, data={!r})".format(self._xml_class,
                self._featname2merge_op or merge_op, dict(self._dict))
    ...
    def __iter__(self):
        for featname, value in self._dict.items():
    ...
    def replace_feature(self, featname, new_value):
        r"""Replace current (featname, value) pair in this FeatureSet."""
        assert featname is not None
